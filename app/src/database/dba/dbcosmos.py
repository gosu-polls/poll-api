...
class CosmosDb:
    # Create/Update/Delete a Container
    # Create/Update/Delete an Item inside a Container
    ...
    def __new__(cls):
        ...
        # _client = cosmos_client.CosmosClient(url=HOST, 
        #                                      credential={'masterKey': MASTER_KEY}, 
        #                                      user_agent="CosmosDBPythonQuickstart", 
        #                                      user_agent_overwrite=True)

            cls._client = cosmos_client.CosmosClient(os.getenv("COSMOSDB_HOST"), 
                                             {'masterKey': os.getenv("COSMOSDB_MASTER_KEY")})
            cls._dbInstance = cls._client.get_database_client(os.getenv('COSMOSDB_POLLS_DB_ID'))
        return cls._instance
    
    # @classmethod
    # def CreateDB(cls, database_name):
    #     try:
    #         db = cls._client.create_database(id=database_name)
    #     except exceptions.CosmosResourceExistsError:
    #         db = cls._client.get_database_client(database_name)

    #     cls._dbMap[database_name] = db

    # @classmethod
    # def DeleteDB(cls, database_name):
    #     try:
    #         cls._client.delete_database(cls._dbMap[database_name])
    #         cls._dbMap.pop(database_name)

    #     except exceptions.CosmosResourceNotFoundError:
    #         pass

    @classmethod
    # def CreateContainer(cls, database_name, container_name, partition_key):
    def CreateContainer(cls, container_name, partition_key = '/partitionKey'):
        container_name = container_name.lower()
        try:
            # container = cls._dbMap[database_name].create_container(id=container_name, 
            #                                                        partition_key=PartitionKey(path='/partitionKey'))
            container = cls._dbInstance.create_container(id=container_name, 
                                                         partition_key=PartitionKey(path=partition_key))
            
            
        except exceptions.CosmosResourceExistsError:
            # container = cls._dbMap[database_name].get_container_client(container_name)
            container = cls._dbInstance.get_container_client(container_name)

        # cls._containerMap[database_name][container_name] = container
        return container
    ...
    def AddData(cls, identifier: dict = {}, data: dict = {}):
        data['id'] = identifier['item_identifier'] + '~' + data[identifier['partition_key']]
        data['partitionKey'] = data[identifier['partition_key']]
...
# HOST = 'https://polls-db.documents.azure.com:443/'
# MASTER_KEY = 'EezxrdEC98s5KxMmoNFO4ciJ6Tf5nW8aDEFipzTbMshqaf3j8682Kd70i29a2D2OdcyysPl6Ol40ACDbZV1Nwg=='
